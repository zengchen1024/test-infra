...
import (
	...
	giteeTrigger "k8s.io/test-infra/prow/gitee-plugins/trigger"
...
type githubClient interface {
	CreateStatus(string, string, string, int, github.Status) error
	GetCombinedStatus(org, repo, ref string, number int) (*github.CombinedStatus, error)
...
func NewController(ghcSync, ghcStatus githubClient, mgr manager, cfg config.Getter, gc git.ClientFactory, maxRecordsPerPool int, opener io.Opener, historyURI, statusURI string, logger *logrus.Entry) (*Controller, error) {
	if logger == nil {
...
func (c *Controller) trigger(sp subpool, presubmits []config.Presubmit, prs []PullRequest) error {
	...
	// add annotation to presubmit
	giteeTrigger.SetPresubmit(sp.org, sp.repo, presubmits)
...
func (c *Controller) dividePool(pool map[string]PullRequest) (map[string]*subpool, error) {
	...
	for subpoolkey, sp := range sps {
		...
		items := filterProwJob(pjs.Items)
		c.logger.WithField("subpool", subpoolkey).Debugf("Found %d prowjobs.", len(items))
		sps[subpoolkey].pjs = items
	}
	return sps, nil
}
...
func headContexts(log *logrus.Entry, ghc githubClient, pr *PullRequest) ([]Context, error) {
	...
	combined, err := ghc.GetCombinedStatus(org, repo, string(pr.HeadRefOID), int(pr.Number))
	if err != nil {
